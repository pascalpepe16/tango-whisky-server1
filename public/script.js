...
function processFile(){
  ...
  const previewArea=document.getElementById("genPreview"); // preview dans Créer / Ajouter
...
document.getElementById("genForm").addEventListener("submit",e=>{
  e.preventDefault();
  const form=e.target;
  const preview=document.getElementById("genPreview"); // preview réduit
  const formData=new FormData(form);
  const imgFile=formData.get("qsl");
  const data=Object.fromEntries(formData.entries());
  const imgURL=URL.createObjectURL(imgFile);
  preview.innerHTML=generateQSLPreview(data,imgURL); // injecte dans preview réduit
