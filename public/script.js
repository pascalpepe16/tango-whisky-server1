...
function generateQSLPreview(data, imageUrl){
  return `<div class="qsl-card">
    <div class="qsl-image"><img src="${imageUrl}" alt="QSL"></div>
    <div class="qsl-text">
...
function processFile(){
  ...
  if(ext==="csv"){Papa.parse(file,{header:true,skipEmptyLines:true,complete:function(results){
    importedLogs=results.data.map(normalizeRow).filter(r=>r.Indicatif!=="");
    status.innerText=`${importedLogs.length} lignes valides chargées`;
    showPreview();
    document.getElementById("validateImportBtn").style.display="inline-block";
  }});}
  else if(ext==="xlsx"){const reader=new FileReader();
    reader.onload=function(e){const data=new Uint8Array(e.target.result);const wb=XLSX.read(data,{type:"array"});const ws=wb.Sheets[wb.SheetNames[0]];const raw=XLSX.utils.sheet_to_json(ws);
...
document.getElementById("validateImportBtn").onclick=async function(){
  ...
  for(let i=0;i<importedLogs.length;i++){
    ...
    try{const res=await fetch(API_URL+"/upload",{method:"POST",body:formData,credentials:"same-origin"});const data=await res.json();if(data.success) success++;}catch(err){}
...
document.getElementById("genForm").addEventListener("submit",async e=>{
  ...
  await fetch(API_URL+"/upload",{method:"POST",body:formData,credentials:"same-origin"});
});
...
document.getElementById("btnSearch").onclick=async function(){
  ...
  try{
    ...
    list.forEach(q=>{
      const div=document.createElement("div");
      div.className="thumbWrap";
      div.innerHTML=`<img src="${q.thumb}"><button onclick="downloadQSL('${q.public_id}')">Télécharger</button>`;
      box.appendChild(div);
    });
  }catch(err){box.innerHTML="Erreur réseau";}
};
function downloadQSL(pid){const a=document.createElement("a");a.href=API_URL+"/file?pid="+encodeURIComponent(pid);a.click();}
