...
// IMPORT CSV / XLSX
// ===============================
document.getElementById("importBtn").onclick=processFile;
function processFile(){
  ...
  if(ext==="csv"){
    ...
    reader.onload=function(e){
      ...
      importedLogs=XLSX.utils.sheet_to_json(ws).map(normalizeRow).filter(r=>r.Indicatif!=="");
      status.innerHTML=`${importedLogs.length} lignes valides chargées`;
      showPreview();
      document.getElementById("validateImportBtn").style.display="inline-block";
    };
...
document.getElementById("btnSearch").addEventListener("click",async ()=>{
  ...
  try{
    ...
    list.forEach(q=>{
      const div=document.createElement("div");
      div.className="thumbWrap";
      div.innerHTML=`<img src="${q.thumb}" class="dlThumb">
                     <button onclick="downloadQSL('${q.public_id}')">Télécharger</button>`;
      box.appendChild(div);
    });
  }catch(err){ console.error(err); box.innerHTML="Erreur réseau"; }
});

function downloadQSL(pid){
  const a=document.createElement("a");
  a.href=API_URL+"/file?pid
