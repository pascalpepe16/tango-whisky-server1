...
function processFile(){
  ...
  let imageURL="";
  if(imageInput.files[0]) imageURL=URL.createObjectURL(imageInput.files[0]);
  ...
  const showPreview=()=>{ previewArea.innerHTML=""; importedLogs.slice(0,10).forEach(r=>{ previewArea.innerHTML+=generateQSLPreview(r,imageURL); }); };

  if(ext==="csv"){
    Papa.parse(file,{header:true,skipEmptyLines:true,complete: r=>{ importedLogs=r.data.map(normalizeRow).filter(r=>r.Indicatif!==""); status.innerText=`${importedLogs.length} lignes valides chargées`; showPreview(); document.getElementById("validateImportBtn").style.display="inline-block"; } });
  }else if(ext==="xlsx"){
