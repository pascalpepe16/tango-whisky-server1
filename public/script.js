...
async function login(){
  ...
  const errBox = document.getElementById("loginError"); errBox.innerText="";
  try{
...
function processFile(){
  ...
  const file = fileInput.files[0]; if(!file) return alert("Choisissez un fichier");
  ...
  if(ext==="csv"){
    Papa.parse(file,{header:true,skipEmptyLines:true,complete:results=>{
      ...
      showPreview(); document.getElementById("validateImportBtn").style.display="inline-block";
    }});
  }else if(ext==="xlsx"){
...
document.getElementById("validateImportBtn").onclick=async function(){
  ...
  progress.style.display="block"; bar.style.width="0%"; let success=0;
